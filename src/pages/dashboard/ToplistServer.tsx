...
import { CheckCircle2, ChevronDown, Clipboard, ExternalLink, Loader2, Send, Trophy, XCircle } from "lucide-react"
...
function CallbackSetup({
  ...
  const hasUnsavedUrl = callbackUrl.trim() !== (server.callback_url ?? "").trim()
  ...
  return (
    <section className="mt-6 border border-border/40 rounded-lg overflow-hidden">
      <div className="px-5 py-4 border-b border-border/40 bg-secondary/15">
        <h2 className="text-base font-semibold text-foreground">Callback Setup &amp; Tester</h2>
        <p className="text-xs text-muted-foreground mt-1">Receive a request after every confirmed vote, then use the tester to make sure your endpoint can be reached.</p>
      </div>

      <div className="p-5 space-y-3">
        <div className="rounded border border-border/50 bg-secondary/10 p-4">
          <h3 className="text-sm font-semibold text-foreground">Your server vote URL</h3>
          <p className="mt-1 text-xs text-muted-foreground">Use this working ScapePulse URL on your server's vote button. Replace <code className="text-foreground">{'{sid}'}</code> with your listing ID and <code className="text-foreground">{'{incentive}'}</code> with the unique incentive you create for that player.</p>
          <code className="mt-3 block break-all rounded bg-background/70 px-3 py-2 text-xs text-foreground">https://scapepulse.com/toplist/vote/{'{sid}'}/{'{incentive}'}</code>
        </div>

        <p className="text-sm text-muted-foreground">ScapePulse calls your URL with <code className="text-foreground">uid</code> and <code className="text-foreground">voter_name</code>. URLs that already have parameters are supported, for example <code className="text-foreground break-all">https://example.com/api/vote/callback?postback=</code>.</p>

        <details className="group rounded border border-border/50 bg-secondary/10">
          <summary className="flex cursor-pointer list-none items-center justify-between px-4 py-3 text-sm font-medium text-foreground">
            PHP callback example <ChevronDown className="h-4 w-4 text-muted-foreground transition-transform group-open:rotate-180" />
          </summary>
          <CodeExample value={phpCallbackExample} copied={copied === "php"} onCopy={() => copy("php", phpCallbackExample)} />
        </details>

        <details className="group rounded border border-border/50 bg-secondary/10">
          <summary className="flex cursor-pointer list-none items-center justify-between px-4 py-3 text-sm font-medium text-foreground">
            Supabase Edge Function example <ChevronDown className="h-4 w-4 text-muted-foreground transition-transform group-open:rotate-180" />
          </summary>
          <CodeExample value={supabaseCallbackExample} copied={copied === "supabase"} onCopy={() => copy("supabase", supabaseCallbackExample)} />
        </details>

        <div className="rounded border border-primary/20 bg-primary/5 p-4">
          <h3 className="text-sm font-semibold text-foreground">Test your saved callback URL</h3>
          <p className="mt-1 text-xs text-muted-foreground">This sends a GET request containing a temporary <code>uid</code> and <code>test=1</code>. It never creates a vote or grants a reward.</p>
          {!server.callback_url?.trim() ? (
            <p className="mt-3 text-sm text-amber-300">Add a callback URL above and save your listing before testing it.</p>
          ) : hasUnsavedUrl ? (
            <p className="mt-3 text-sm text-amber-300">Save your changed callback URL before testing it.</p>
          ) : (
            <div className="mt-3 flex flex-wrap items-center gap-3">
              <button type="button" onClick={test} disabled={testing}
                className="inline-flex items-center gap-2 rounded bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50">
                {testing ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Send className="h-3.5 w-3.5" />}
                {testing ? "Testing…" : "Send Test Callback"}
              </button>
              {result && (
                <p className={`flex items-center gap-1.5 text-xs ${result.delivered ? "text-emerald-400" : "text-red-300"}`}>
                  {result.delivered ? <CheckCircle2 className="h-4 w-4" /> : <XCircle className="h-4 w-4" />}
  ...
  return (
    <div className="border-t border-border/40 p-3">
      <div className="mb-2 flex justify-end">
        <button type="button" onClick={onCopy} className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground">
          <Clipboard className="h-3.5 w-3.5" /> {copied ? "Copied" : "Copy code"}
        </button>
      </div>
      <pre className="overflow-x-auto rounded bg-background/70 p-3 text-xs leading-5 text-foreground"><code>{value}</code></pre>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <CallbackSetup server={server} callbackUrl={form.callback_url} />
