  ...
  return (
    <section className="border border-border/70 bg-card/70 p-2.5 shadow-lg shadow-black/10 backdrop-blur-sm">
      <div className="mb-2 flex items-center gap-2 px-0.5">
        {isVotes ? <Trophy className="h-4 w-4 text-amber-300" /> : <span className="flex h-4 w-4 items-center justify-center border border-primary/50 text-[10px] text-primary">+</span>}
      ...
      <div className="space-y-1.5">
        {servers.length === 0 ? (
          ...
          <article key={server.id} className="group relative min-h-14 overflow-hidden border border-border/55 bg-background/40 p-1.5 transition-colors hover:border-primary/50 hover:bg-secondary/45">
            <div className="flex items-center gap-2 transition-opacity group-hover:opacity-20">
              <span className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden border border-primary/20 bg-card">
            ...
            <div className="absolute inset-0 flex items-center justify-center gap-1 bg-card/92 px-1 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
              <Link to={`/toplist/servers/${server.id}`} className="inline-flex items-center gap-1 bg-primary px-1.5 py-1.5 text-[9px] font-bold text-primary-foreground hover:brightness-110">
                <Play className="h-3 w-3 fill-current" /> Play
              </Link>
              {server.discord_invite && <a href={server.discord_invite} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 border border-primary/40 px-1.5 py-1.5 text-[9px] font-bold text-primary hover:bg-primary/10">
                <MessageCircle className="h-3 w-3" /> Discord
              </a>}
              {!server.discord_invite && <span title="Discord link not provided" className="inline-flex cursor-not-allowed items-center gap-1 border border-border px-1.5 py-1.5 text-[9px] font-bold text-muted-foreground">
                <MessageCircle className="h-3 w-3" /> Discord
              </span>}
            </div>
          </article>
